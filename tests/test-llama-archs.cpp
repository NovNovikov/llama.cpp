...
static gguf_context_ptr get_gguf_ctx(
        const llm_arch arch, const bool moe, const bool qwen_ple = false, const bool nextn = false) {
...
static int test_mtp(const size_t seed, const ggml_log_level log_level) {
    ...
    gguf_context_ptr gguf_ctx = get_gguf_ctx(LLM_ARCH_GLM5NEXT, true, /*qwen_ple =*/ false, /*nextn =*/ true);
...
int main(int argc, char ** argv) {
    ...
    try {
        ...
        if (arch == LLM_ARCH_UNKNOWN || arch == LLM_ARCH_GLM5NEXT) {
            ret |= test_dsa_kpool(seed, GGML_LOG_LEVEL_INFO);
            ret |= test_mtp(seed, GGML_LOG_LEVEL_INFO);
        }
