...
llama_model_dspark::graph<false>::graph(const llama_model & model, const llm_graph_params & params)
    ...
    // The drafting loop submits full anchor-first blocks so the in-graph Markov head
    // can chain every step from the block anchor through the subsequent predictions.
    if (n_tok % bs != 0) {
        return;
    }
    ...
    ggml_tensor * prev = inp->anchors;
    res->add_input(std::move(inp));

    ggml_tensor * cat = nullptr;
    for (int64_t i = 0; i < bs; ++i) {
        ggml_tensor * bias = ggml_mul_mat(ctx0, w2, ggml_get_rows(ctx0, w1, prev));
