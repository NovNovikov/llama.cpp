...
// Solar Open 2 uses marker-delimited key/value tool arguments instead of JSON.
static common_chat_params common_chat_params_init_solar_open2(const common_chat_template &          tmpl,
    ...
    data.prompt = common_chat_template_direct_apply_impl(tmpl, inputs);
    {
        ...
        const auto pos = data.prompt.rfind(marker);
    ...
    const bool has_tools = inputs.tools.is_array() && !inputs.tools.empty();
    const bool has_response_format = inputs.json_schema.is_object() && !inputs.json_schema.empty();
    const bool include_grammar = has_response_format || (has_tools && inputs.tool_choice != COMMON_CHAT_TOOL_CHOICE_NONE);
        ...
        if (has_tools && inputs.tool_choice != COMMON_CHAT_TOOL_CHOICE_NONE) {
            ...
            foreach_function(inputs.tools, [&](const json & tool) {
                ...
                tool_choice |= p.rule("tool-" + name,
                    p.tool(p.tool_open(p.literal("<|tool_call:start|>") + p.tool_name(p.literal(name)) + p.literal("\n"))
                    << p.tool_args(args)
            ...
            auto tool_calls = p.trigger_rule("tool-call", p.repeat(tool_choice + p.space(), 1, max_calls));
            return generation_prompt + reasoning + p.content(p.until("<|tool_call:start|>")) + tool_calls + p.end();
        }
...
static common_chat_params common_chat_params_init_muse_glimmer(const common_chat_template &          tmpl,
    ...
    const auto extract_reasoning = inputs.reasoning_format != COMMON_REASONING_FORMAT_NONE;
    const auto has_tools = inputs.tools.is_array() && !inputs.tools.empty();
    const auto include_grammar = has_tools && inputs.tool_choice != COMMON_CHAT_TOOL_CHOICE_NONE;
